...
const register = async (req, res) => {
  try {
    ...
    const pictureLocalPath = req.files?.avatar[0]?.path;
    // console.log("req.files: ", req.files);
    console.log("pictureLocalPath: ", pictureLocalPath);

    if (!pictureLocalPath) {
      return res.status(400).json({ message: "User image is required" }); // Successfully created response dena
    }
